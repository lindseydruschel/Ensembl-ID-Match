...
get_kegg_pathways <- function(gene_symbol, species) {
  ...
  kegg_info <- tryCatch({
    kegg_res <- KEGGREST::keggGet(paste0(kegg_species_code, ":", gene_symbol))
    if (length(kegg_res) > 0 && !is.null(kegg_res[[1]]$PATHWAY)) {
      # Extract only the pathway names and limit to top 5
      pathway_names <- head(unlist(kegg_res[[1]]$PATHWAY), 5)
      pathways <- paste(pathway_names, collapse = ", ")
      
      # Print the retrieved pathway names for debugging
      print(paste("Retrieved KEGG pathway names for", gene_symbol, ":", pathways))
      
      return(pathways)
    } else {
      print(paste("No KEGG pathways found for", gene_symbol))
      return(NA)
    }
  }, error = function(e) {
    print(paste("Error retrieving KEGG pathways for", gene_symbol, "-", e$message))
    return(NA)
  })
  ...
  process_input <- function(species, input_type, input_source, user_input, ensembl_input, file_input, keyword) {
    ...
    if (input_type == "gene_symbol") {
      gene_info_results <- do.call(rbind, lapply(gene_ids, function(gene_symbol) {
        ...
        # Wrap in tryCatch to handle individual errors without stopping the entire loop
        tryCatch({
          result <- get_gene_info(gene_symbol, keyword, species)
          return(result)
        }, error = function(e) {
          message(paste("Error processing gene:", gene_symbol, " - ", e$message))
      ...
      gene_info_results <- do.call(rbind, lapply(ensembl_to_gene$external_gene_name, function(gene_symbol) {
        ...
        # Wrap in tryCatch for each gene symbol to continue processing even if an error occurs
        tryCatch({
          result <- get_gene_info(gene_symbol, keyword, species)
          return(result)
        }, error = function(e) {
