...
# Version 75 has diff gene name, remove that
...
process_outdated_ensembl <- function(ensembl_ids, species, version, progress) {
  ...
  progress$set(message = paste("Querying Ensembl version", version, "..."), value = 0)
  ...
  colnames(merged_results) <- c("Gene_Symbol (biomaRt)", "Input (Ensembl v)", 
