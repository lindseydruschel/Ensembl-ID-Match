...
get_gene_info <- function(gene_symbol, keyword, species) {
  ...
  if (is.null(content$hits) || length(content$hits) == 0) {
    # Return NA if no hits were found
    return(data.frame(Gene_Symbol = gene_symbol, Ensembl_ID = NA, Aliases = NA, GO_Terms = NA, KEGG_Pathways = NA))
  }
   hits <- content$hits
  print(hits)
  # Process and extract the first hit if it exists
  if (length(hits) > 0) {
    first_hit <- hits[[1]]  # Access the first hit

    # Since first_hit is a character vector, take it directly
    first_value <- first_hit[1]  # Get the first element
    
    # Print extracted values for debugging
    print(paste("First Value for", gene_symbol, ":", first_value))
    
    # Return the first value
    return(first_value)
  } else {
    print(paste("No hits found for gene:", gene_symbol))
    return(NULL)  # Return NULL if no hits found
  }


  # Extract Ensembl ID
  ensembl_id <- if (!is.null(first_hit$gene)) first_hit$gene else NA
  
  # Flatten Aliases (if available)
  aliases <- if (!is.null(first_hit$alias) && length(first_hit$alias) > 0) {
    paste(unique(unlist(first_hit$alias)), collapse = ", ")
  } else NA
  
  # Extract GO terms (if available)
  go_terms <- if (!is.null(first_hit$go$BP)) {
    terms <- sapply(first_hit$go$BP, `[[`, "term")
    paste(terms, collapse = ", ")
  } else NA
  
  # Query KEGG pathways (if available)
  ...
  process_input <- function(species, input_type, input_source, user_input, ensembl_input, file_input, keyword) {
    ...
    if (input_type == "gene_symbol") {
      gene_info_results <- do.call(rbind, lapply(gene_ids, function(gene_symbol) {
        ...
        print(paste("Processing gene:", gene_symbol))
